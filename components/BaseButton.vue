<template>
  ...
  <NuxtLink v-if="isLink" :to="to" :class="buttonClasses" tabindex="0" v-bind="$attrs" @click="handleNavigation">
    <slot />
  </NuxtLink>
...
const baseClasses =
  "inline-flex items-center justify-center font-medium transition-colors duration-200 focus:outline-none focus:ring-0 focus:ring-offset-0 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer";
...
const variantClasses = computed(() => {
  switch (props.variant) {
    case "primary":
      return "bg-blue-600 text-white hover:bg-blue-700 focus:ring-0"; // Xanh dương
    case "secondary":
      return "bg-gray-600 text-white hover:bg-gray-700 focus:ring-0"; // Xám
    case "ghost":
      return "bg-transparent text-gray-700 hover:bg-gray-100 focus:ring-0 border border-transparent"; // Trong suốt với border transparent
    case "danger":
      return "bg-red-600 text-white hover:bg-red-700 focus:ring-0"; // Đỏ
    case "success":
      return "bg-green-600 text-white hover:bg-green-700 focus:ring-0"; // Xanh lá
    default:
      return "bg-blue-600 text-white hover:bg-blue-700 focus:ring-0"; // Mặc định xanh dương
  }
});
